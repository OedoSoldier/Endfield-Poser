...
- **遗留问题（用户确认"不影响使用"）**：冻结后骨架会有轻微颤抖，表现为在
  "A-pose 静止姿态"和"当前摆姿"之间来回。怀疑是逐帧钉姿势（`Freeze: pose pinned`
  + 从骨快照）和游戏侧某个仍在写的系统（某些角色的从骨链/Twist/corrective、
  或模型替换 mod 的绑定）互相打架。要治先加探针：冻结稳定后连续几帧记录
  Hips/Spine + 一根从骨的 localPos/localRot，看是"两值之间跳"还是"持续漂"。
